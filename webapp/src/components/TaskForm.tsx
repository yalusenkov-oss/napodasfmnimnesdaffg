...
interface TaskFormProps {
  ...
  onSubmit: (task: Omit<Task, 'id' | 'createdAt' | 'completed' | 'updatedAt'>) => void;
  onUpdate?: (id: string, updates: Partial<Task>) => void;
  onClose: () => void;
}

// Опции дополнительного напоминания (в минутах). null = "Не указано"
const REMINDER_OPTIONS = [
  { value: null as number | null, label: 'Не указано' },
...
export function TaskForm({ task, onSubmit, onUpdate, onClose }: TaskFormProps) {
  ...
  // По умолчанию при создании — "Не указано"
  ...
  const getReminderLabel = (minutes: number | null | undefined) => {
  ...
  const handleReminderSelect = (value: number | null) => {
    ...
    if (value === -1) {
      ...
      setReminderMinutes(value as number | null);
    }
                  ...
                  >
                    {REMINDER_OPTIONS.map(({ value, label, isCustom }) => {
                      const isSelected = isCustom
                        ? isCustomReminder
                        : (!isCustomReminder && reminderMinutes === value);
                      return (
                        <button
                          key={value}
                          type="button"
